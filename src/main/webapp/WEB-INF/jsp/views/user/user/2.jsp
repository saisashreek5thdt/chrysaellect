...
<br>
	...
	<div class="row justify-content-md-center " > 
		...
		<!--Syllabus and Videos Section Begin-->
...
<c:forEach items="${rcList}" var="rowt">
      ...
      <c:set var="count" value="${count + 1}" scope="page"/><br style="line-height: .9;">
		<div class="col-md-12" id="sylname">
			...
			<div class="col" id="syllabus">
				<font style="position: relative;left:8%;top: 30%;color:#0A62A3">
				<form action="/rcdetails" method="get">
				<input type="hidden" name="course" value="${row.coursename}">
				<input type="hidden" name="resource" value="${rowt.resourceid}">
					<button style="color:#0A62A3;background:none;height:100%;width:40%">  ${rowt.resourceid}</button></font>
</form>
			</div>
	...
	<div class="col-md-9" >
		<c:forEach items="${rcList}" var="rowk">
			...
			<c:set var="coun" value="${coun + 1}" scope="page"/>
			 ...
			 <c:forEach items="${resourceList}" var="rowm">
				 ...
				 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.filetype=='video' }">
		...
		Videos
			...
			<center>
        					<font style="color:#0A62A3;font-size: 10px;">
...
View All
			...
			<div class="col-md-12" >
				...
				<div class="row">
					...
					<c:forEach items="${rcList}" var="rowk">
						<c:if test="${rowk.coursename==row.coursename}">
						<c:set var="counv" value="${counv + 1}" scope="page"/>
						 <c:if test="${counv<'2'}">
						 <c:forEach items="${resourceList}" var="rowm">

							 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.filetype=='video' }">
							  <c:set var="cz" value="${cz + 1}" scope="page"/>
							  <c:if test="${cz<5}">

								<div class="col-md-3">
									<div class="col-md-12" id="videoout"  >
											<center>
												...
												<div class="col-md-11" id="videoout1"  >
													<form action="/viewvideo" method="get">
                                                									<input type="hidden" value="${rowm.filename}" name="video">
                                                										...
                                                										<input type="hidden" value="${row.coursename}" name="cn">
                                                									<button>
							...
							<div class="col-md-12">
                        						<div class="col-md-12" style="border-top: 5px solid #0A62A3;"  >
	...
	<c:forEach items="${rcList}" var="rowk">
		<c:if test="${rowk.coursename==row.coursename}">
		<c:set var="con" value="${con + 1}" scope="page"/>
		 <c:if test="${con<'2'}">
		 <c:forEach items="${resourceList}" var="rowm">

			 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Worksheets'  }">
...
<div class="col-md-12" >
				...
				<div class="row">
					...
					<c:forEach items="${rcList}" var="rowk">
						<c:if test="${rowk.coursename==row.coursename}">
						<c:set var="counvq" value="${counvq + 1}" scope="page"/>
						 <c:if test="${counvq<'2'}">
						 <c:forEach items="${resourceList}" var="rowm">

							 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Worksheets' }">
							  <c:set var="czq" value="${czq + 1}" scope="page"/>
							  <c:if test="${czq<5}">
 ...
 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.filetype=='ppt' }">
<a href="../assets/uploads/ppt/${rowm.filename}" style="color:#0A62A3;font-size:12px" target="_blank">
 ...
 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.filetype=='pdf' }">
<a href="../assets/uploads/pdf/${rowm.filename}" style="color:#0A62A3;font-size:12px" target="_blank">
 ...
 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.filetype=='word' }">
<a href="../assets/uploads/word/${rowm.filename}" style="color:#0A62A3;font-size:12px" target="_blank">
	<img src="../assets/user/images/doc.png" style="height: 70%;">
			...
			<c:forEach items="${rcList}" var="rowk">
				<c:if test="${rowk.coursename==row.coursename}">
				<c:set var="cok" value="${cok + 1}" scope="page"/>
				 <c:if test="${cok<'2'}">
				 <c:forEach items="${resourceList}" var="rowm">

					 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Activity '  }">
...
<div class="col-md-12" >
				...
				<div class="row">
					...
					<c:forEach items="${rcList}" var="rowk">
						<c:if test="${rowk.coursename==row.coursename}">
						<c:set var="counvqq" value="${counvqq + 1}" scope="page"/>
						 <c:if test="${counvqq<'2'}">
						 <c:forEach items="${resourceList}" var="rowm">

							 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Activity ' }">
							  <c:set var="czqq" value="${czqq + 1}" scope="page"/>
							  <c:if test="${czqq<5}">
...
<a href="../assets/uploads/word/${rowm.filename}" style="color:#0A62A3;font-size:12px" target="_blank">
	<img src="../assets/user/images/doc.png" style="height: 70%;">
			...
			<c:forEach items="${rcList}" var="rowk">
				<c:if test="${rowk.coursename==row.coursename}">
				<c:set var="cqn" value="${cqn + 1}" scope="page"/>
				 <c:if test="${cqn<'2'}">
				 <c:forEach items="${resourceList}" var="rowm">

					 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Articles'  }">
					  <c:set var="cq" value="${cq + 1}" scope="page"/>
					  <c:if test="${cq<2}">
			<div class="row">
		...
		<center>
    					...
    					<b>
  						   ...
  						   <button style="background:none;color:#0A62A3;font-size: 10px;">
View All
</button>
    						</center>
				</div>
...
<div class="col-md-12" >
				...
				<div class="row">
					...
					<c:forEach items="${rcList}" var="rowk">
						<c:if test="${rowk.coursename==row.coursename}">
						<c:set var="counvqz" value="${counvqz+ 1}" scope="page"/>
						 <c:if test="${counvqz<'2'}">
						 <c:forEach items="${resourceList}" var="rowm">

							 <c:if test="${rowm.resource_name==rowk.resourceid && rowm.category=='Articles' }">
							  <c:set var="czqz" value="${czqz + 1}" scope="page"/>
							  <c:if test="${czqz<5}">
